...
const data = buildTestValleyData();
...
const spawnBox = { x: TEST_VALLEY_SPAWN.x - 5, y: TEST_VALLEY_SPAWN.y - 8, w: 10, h: 8 };
ok(!overlapsSolid(spawnBox, map), "spawn point is walkable");

// Flood fill from spawn over walkable tiles.
const sx = Math.floor(TEST_VALLEY_SPAWN.x / 16);
const sy = Math.floor(TEST_VALLEY_SPAWN.y / 16);
...
// Pond is solid water; map border is solid.
ok(map.isSolid(27, 3), "pond water is solid");
ok(map.isSolid(0, 15) && map.isSolid(39, 15) && map.isSolid(20, 0) && map.isSolid(20, 29), "tree border is solid");
...
{
  const box = { x: TEST_VALLEY_SPAWN.x - 5, y: TEST_VALLEY_SPAWN.y - 8, w: 10, h: 8 };
